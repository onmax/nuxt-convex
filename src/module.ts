...
import { formatProjectRelativePath, resolveConvexRoot, toImportPath } from './internal/convex-paths'
...
function setupConvexApiAlias(nuxt: Nuxt): void {
  ...
  const generatedDir = join(convexRoot.readDir, '_generated')
...
async function scaffoldStorageFunctions(nuxt: Nuxt): Promise<void> {
  ...
  if (convexRoot.fallsBackToProjectWrite) {
    log.warn(`Resolved ${formatProjectRelativePath(nuxt, convexRoot.readDir)} from an unsafe layer. Scaffolding ${formatProjectRelativePath(nuxt, storagePath)} instead. Set \`convex.dir\` to a writable project path to override this.`)
  }
