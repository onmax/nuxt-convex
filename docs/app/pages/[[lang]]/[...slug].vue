...
const headline = ref(toHeadlineString(findPageHeadline(navigation?.value, page.value?.path)))
const breadcrumbs = computed(() => findPageBreadcrumbs(navigation?.value, page.value?.path || ''))

function toHeadlineString(value: unknown): string | undefined {
  if (!value) return undefined
  if (typeof value === 'string') return value
  if (typeof value === 'object' && value !== null && 'title' in value) return String((value as Record<string, unknown>).title)
  return undefined
}
...
watch(() => navigation?.value, () => {
  headline.value = toHeadlineString(findPageHeadline(navigation?.value, page.value?.path)) || headline.value
})
