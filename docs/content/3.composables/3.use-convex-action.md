...
const { execute: summarize, isPending, error } = useConvexAction(api.ai.summarize)
...
<template>
  <div>
    <button :disabled="isPending" @click="handleSummarize('Some text to summarize')">
      {{ isPending ? 'Processing...' : 'Summarize' }}
    </button>
    <p v-if="result">
      {{ result }}
    </p>
...
| `isPending` | `Ref<boolean>`         | True while running     |
...
const { execute: processPayment, isPending } = useConvexAction(api.payments.process)
