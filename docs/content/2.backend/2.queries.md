...
::note
Queries are read-only and cannot modify data. Use [mutations](/backend/mutations) to write data.
::
...
const { data: tasks, isPending } = useConvexQuery(api.tasks.list, { userId: 'user_123' })
</script>
