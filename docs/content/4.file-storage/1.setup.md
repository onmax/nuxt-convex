...
When your app extends a Nuxt layer, the first layer that defines `convex.dir` stays authoritative. If that layer comes from `node_modules`, the scaffold is written to your project layer and its imports still point back to the authoritative generated files. Use `convex.dir` if your Convex files live somewhere other than `convex/`.
