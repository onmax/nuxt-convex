...
function setupConvexExports(nuxt: Nuxt, resolve: (path: string) => string): void {
  const convexVueRuntimeImport = resolveConvexVueImport(nuxt, resolve, join(nuxt.options.buildDir, 'convex'))
  const convexVueTypeImport = resolveConvexVueImport(nuxt, resolve, join(nuxt.options.buildDir, 'types'))
...
function setupConvexStorage(nuxt: Nuxt, resolve: (path: string) => string): void {
  ...
  const template = addTemplate({
    ...
    getContents: () => `// Auto-generated by nuxt-convex
export { useConvexStorage } from '${convexVueRuntimeImport}'
`,
    write: true,
  })
...
declare module '#convex/storage' {
  export { useConvexStorage } from '${convexVueTypeImport}'
}
`,
  }, { nitro: true, nuxt: true })
...
function resolveConvexVueImport(nuxt: Nuxt, resolve: (path: string) => string, fromDir: string): string {
  const localSourceEntry = resolve('../../vue/src/index')
